...
func main() {
	...
	redisClient, err := db.NewRedisClient(config.Database.RedisAddr, logr)
	if err != nil {
		logr.Fatal().Err(err).Msg("failed to initialize redis client")
	}
	defer redisClient.Close()

	if err = redisClient.Ping(context.Background()); err != nil {
		logr.Fatal().Err(err).Msg("failed to ping redis connection")
	}
	...
	go workers.RunTaskProcessor(logr, store, redisClient, mailer, config, redisOpt, emailTemplate)
